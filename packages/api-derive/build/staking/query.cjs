...
function query(instanceId, api) {
  return (0, _index.memo)(instanceId, (accountId, flags) => api.derive.staking.queryMulti([accountId], flags).pipe((0, _rxjs.map)(_ref4 => {
    let [first] = _ref4;
    return first;
  })));
}
...
function queryMulti(instanceId, api) {
  return (0, _index.memo)(instanceId, (accountIds, flags) => accountIds.length ? api.derive.session.indexes().pipe((0, _rxjs.switchMap)(_ref5 => {
    let {
      activeEra
    } = _ref5;
