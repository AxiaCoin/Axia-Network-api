...
function signingHeader(api) {
  return (0, _rxjs.combineLatest)([api.rpc.chain.getHeader(), api.rpc.chain.getFinalizedHead()]).pipe((0, _rxjs.switchMap)(_ref2 => {
    let [bestHeader, finHash] = _ref2;
    return (// retrieve the headers - in the case of the current block, we use the parent
      ...
      bestHeader.parentHash.isEmpty ? (0, _rxjs.of)([bestHeader, bestHeader]) : (0, _rxjs.combineLatest)([api.rpc.chain.getHeader(bestHeader.parentHash), api.rpc.chain.getHeader(finHash)])
    );
  }), (0, _rxjs.map)(_ref3 => {
    let [current, finalized] = _ref3;
...
function signingInfo(_instanceId, api) {
  ...
  (0, _util.isUndefined)(era) || (0, _util.isNumber)(era) && era > 0 ? signingHeader(api) : (0, _rxjs.of)(null)]).pipe((0, _rxjs.map)(_ref4 => {
    var _api$consts$system, _api$consts$system$bl, _api$consts$babe, _api$consts$timestamp;

    let [nonce, header] = _ref4;
    return {
      header,
