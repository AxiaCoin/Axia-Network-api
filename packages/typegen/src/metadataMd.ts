...
import type { MetadataLatest, PortableRegistry, SiLookupTypeId } from '@axia-js/types/interfaces';
import type { Codec, DefinitionRpcParam } from '@axia-js/types/types';
...
function addStorage ({ lookup, pallets, registry }: MetadataLatest): string {
  const { axlib } = getAxlibStorage(registry);
  ...
  return renderPage({
    ...
    sections: moduleSections.concat([{
      ...
      items: Object.entries(axlib).map(([name, { meta }]) => {
