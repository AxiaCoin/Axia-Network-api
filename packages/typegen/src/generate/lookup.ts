...
import type { PortableRegistry, PortableType, SiLookupTypeId, SiPath, SiTypeParameter } from '@axia-js/types/interfaces';
  ...
  const named = lookup.types.filter(({ id, type: { path } }) => {
    ...
    return (
      ...
      !(
        ...
        (
          (
            // Ensure that we match {node, axialunar, *}_runtime
