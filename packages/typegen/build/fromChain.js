...
function generate(metaHex, pkg, output, isStrict) {
  ...
  writeFile(path.join(process.cwd(), output, 'augment-api.ts'), () => [HEADER('chain'), ...['@axia-js/api/augment/rpc', ...['consts', 'errors', 'events', 'query', 'tx', 'rpc'].filter(key => !!key).map(key => `./augment-api-${key}`)].map(path => `import '${path}';\n`)].join(''));
  process.exit(0);
}
