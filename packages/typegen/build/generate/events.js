    ...
    const modules = pallets.filter(({
      ...
      items: lookup.getSiType(events.unwrap().type).def.asVariant.variants.map(({
        ...
        const args = fields.map(({
          type
        }) => lookup.getTypeDef(type)).map(typeDef => typeDef.lookupName || formatType(registry, allDefs, typeDef, imports));
...
export function generateDefaultEvents(dest = 'packages/api/src/augment/events.ts', data, extraTypes = {}, isStrict = false) {
  const {
    metadata
