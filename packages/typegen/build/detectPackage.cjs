"use strict";

var _packageInfo = require("@axia-js/api/packageInfo");

var _packageInfo2 = require("@axia-js/rpc-provider/packageInfo");

var _packageInfo3 = require("@axia-js/types/packageInfo");

var _util = require("@axia-js/util");

var _packageInfo4 = require("./packageInfo.cjs");

// Copyright 2017-2021 @axia-js/typegen authors & contributors
// SPDX-License-Identifier: Apache-2.0
(0, _util.detectPackage)(_packageInfo4.packageInfo, typeof __dirname !== 'undefined' && __dirname, [_packageInfo.packageInfo, _packageInfo2.packageInfo, _packageInfo3.packageInfo]);