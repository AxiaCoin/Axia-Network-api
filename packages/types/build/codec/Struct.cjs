...
function decodeStructFromObject(registry, Types, value, jsonMap) {
  ...
  (0, _util.assert)(!Array.isArray(value) || value.length === inputKeys.length, () => `Struct: Unable to map ${(0, _util.stringify)(value)} array to object with known keys ${inputKeys.join(', ')}`);
  return inputKeys.reduce((raw, key, index) => {
    ...
    try {
      if (Array.isArray(value)) {
        ...
        throw new Error(`Cannot decode value ${(0, _util.stringify)(value)} (typeof ${typeof value}), expected an input object with known keys`);
      }
    } catch (error) {
      let type = Type.name;
...
function decodeStruct(registry, Types, value, jsonMap) {
  if ((0, _util.isHex)(value)) {
    ...
    const values = (0, _index.decodeU8a)(registry, value, Object.values(Types), keys); // Transform array of values to {key: value} mapping
...
class Struct extends Map {
  ...
  toRawType() {
    return (0, _util.stringify)(Struct.typesToMap(this.registry, (0, _classPrivateFieldLooseBase2.default)(this, _Types)[_Types]));
  }
  /**
