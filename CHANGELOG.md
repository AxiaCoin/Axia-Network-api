...
Upgrade priority: Low. Recommended for all users on 6.0.1, especially those using `api.query.axlib.*` calls

Changes:

- Fix `api.query.axlib.*` key generation


## 6.0.1 Sep 18, 2021

Upgrade priority: Low. Recommended for users of the latest Axlib/AXIA with Metadata v14.
...
- Additional types for allychain Disputes
...
- Adjust Statemint, BetaNet and AlphaNet Xcm overrides
...
- Adjust AXIALunar Xcm overrides
...
- Adjust aliases for new allychain module locations
...
- Add AXIALunar 9090 upgrade block


## 5.6.1 Aug 23, 2021

Upgrade priority: Low. Recommended for users of Xcm, preparing for AXIALunar/AlphaNet 9100
...
- Bump static Axlib, AXIALunar & AXIA metadata


## 5.5.2 Aug 19, 2021

Upgrade priority: Medium. Contains fixes for allychain types, affecting historic block retrieval on AXIALunar.
...
- Align static Metadata with latest Axlib, AXIA & AXIALunar
...
- Align static metadata with latest Axlib
...
- Perform sorting on `BTree{Map, Set}`, aligning with Axlib (Thanks to https://github.com/Lezek123)
...
- Align Xcm Junction type with Axlib (Thanks to https://github.com/dt665m)

Changes:

- Update `ParaPastCodeMeta` with `ReplacementTimes`
- Add mappings for `AccountId20` & `AccountId32` types
- Adjust scale-info types with Axlib (mapping for api-contracts to old)
- Allow nested structs in enum definitions & generation
- Allow for parsing of v14 metadata (not enabled by default, not merged into Axlib as of yet)


## 5.0.1 Jul 11, 2021

Upgrade priority: Low. Recommended for users of current-generation Axlib chains.

- **Breaking change** The `@axia-js/metadata` package has been removed, adjust imports to `@axia-js/types`
- **Important** Internally rxjs has been upgraded to the 7.0 version, affecting users of the `ApiRx` interface

Changes:

- Remove the `@axia-js/metadata` package, consolidate into `@axia-js/types`
...
Upgrade priority: Low. Recommended for full from-genesis queries on AXIALunar.
...
- Add missing historic `Heartbeat: HeartbeatTo244` mapping to AXIALunar
...
- Add upgrade block for AXIALunar 9040
- Use `BN` imports from `@axia-js/util`


## 4.13.1 Jun 6, 2021

Upgrade priority: Medium. Required for users connecting to Statemine and future AXIALunar upgrades.
...
- Added 9031-9033 upgrade block for AlphaNet
- Update static metadata to latest Axlib
...
- Align identity `Data` to limit to 32 bytes in the `Raw` enum (as per Axlib)
- Align static (test & docs-only) metadata with latest Axlib master
- Added 9030 upgrade blocks for AXIALunar & AlphaNet
- Cleanup of Eth pallet RPC definitions
- Cleanup README for `@axia-js/rpc-core` (recommended for internal-usage only)
...
Upgrade priority: Low. Recommended for users using staking entries on AXIA and users of Axlib master.
...
- Adjust AlphaNet types for 16 nominators-only
...
- Cleanup BetaNet known types (only as used)
...
- Allow for `Bounded*` type usage (as per latest Axlib)
...
- Remove support for the old-style V2 contracts from `@axia-js/api-contract`
...
- Add AXIALunar & AlphaNet 9010 upgrade blocks
- Adjust `CompactSolution` for 24 nominators
- Bump static metadata to latest Axlib


## 4.9.1 May 6, 2021

Upgrade priority: Medium. Required for the next v0.9.0 AXIA/AXIALunar chain upgrades where allychains are used.
...
- Adjust allychain types
- Remove error logging on createType (details passed in error)
- Adjust `ContractInfo` type as per latest Axlib
- Adjust `DigestItem` type as per latest Axlib
...
- Add known upgrade block for AlphaNet 900 & 9000
- Remove all deprecated interfaces inside `@axia-js/api-contracts`
- Initial support for NMap storage keys
- Support metadata v13 from Axlib (upcoming upgrade)
- Optimize historic metadata retrieval from rpc core


## 4.8.1 May 2, 2021

Upgrade priority: Low. Recommended when using or developing for allychains.
...
- Update to latest Axlib metadata
...
Upgrade priority: Low. Recommended for users following Axlib master.
...
- Adjust `DispatchError` type as per current Axlib master
...
- Add AXIALunar upgrade block for 2030


## 4.5.1 Apr 12, 2021

Upgrade priority: Low. Recommended with usage with the BetaNet testnet.

Changes:

- Update allychain types as per latest AXIA
...
- Align types with latest Axlib master
...
Upgrade priority: Low. Recommended when tracking the latest updates from Axlib.

Contributed:

- Fix contract even decoding on Axlib 3 (Thanks to https://github.com/vminkov)

Changes:

- Adjust `Extender{Header, SignedBlock}` types to ensure at-block `Header` types are used
- Explicit definition for new BetaNet triple refcount `AccountData` types
...
- Bump metadata to latest axlib version
...
- Add upgrade block for AXIALunar 2029
...
- Align types with latest axlib master
- Apply latest AXIA auctions/crowdloan/allychains/slots types
...
- Fix import from `@axia-js/x-rxjs` in Node environments (From common)
...
- Enum `.toSON()` now encodes all keys as `camelCase` (aligning with the output from Axlib)
...
- Add `AccountInfoWithTripleRefCount` to cater for latest Axlib
- Add upgrade block for AlphaNet 49
...
- Update election types as per latest Axlib
- Add types for the new gilt module
- Upgrade to the latest Axlib metadata
...
- Update for latest Axlib contracts `Schedule` types
...
Upgrade priority: Low. Recommended for allychain builders since it contains the updated allychain types.

Changes:

- Adjusted allychain `PersistedValidationData` structure


## 3.9.2 Feb 15, 2021

Upgrade priority: Low. Recommended for allychain builders since it contains the latest allychain types.
...
- Added missing Culmulus `AllychainInherentData` & `MessageQueueChain` types
...
- Adjust council derives to cater for latest Axlib & AXIA
- Adjust BetaNet know type definitions to cater for the latest update
- Update types for latest AXIA/Axlib
- Add generic arguments for TypeScript users to `.entries/.keys` to deal with the key typings


## 3.8.1 Feb 7, 2021

Upgrade priority: Low. Recommended for users of the latest Axlib master, especially using contracts.

- **Important** The `api-contract` `Code` now supports deploying the code and a contract in on operation. This aligns with the latest Axlib where code cannot be uploaded with a contract. The `createBlueprint` is therefore deprecated, use `code.tx.<constructor>(...)` to deploy code. (Compatible with both old and new versions of Axlib)
...
- Expand `api-contract` to allow for `Code` to deploy contract alongside code. `createBlueprint` is now deprecated, replaced by `createContract` or the preferred `code.tx.<constructor>`, which uploads and deploys the code in one operation. This aligns with the current Axlib master contracts implementation.
...
- Updated to latest Axlib metadata
...
Upgrade priority: Low. However recommended for current Axlib master, AXIA and BetaNet users and those wishing to support the upcoming AXIA 28 runtime with the new `MultiAddress`.
...
- Update all allychain types (as per latest BetaNet)
- Correctly use relay blockNumber in allychain validation data
...
- Update types & metadata to latest Axlib
- Move the `@axia-js/x-rxjs` package into the common repo
- Allow doc generation for errors/events (markdown outputs)


## 3.5.1 Jan 18, 2020

Upgrade priority: Low. Recommended for allychain developers.

- **Important** The default for Axlib on the Address types are `MultiAddress`. It is recommended that chains add explicit definitions for `Address` and `LookupSource` in their types, instead of relying on the API-defaults. A future update will swap the API defaults to align with Axlib.
- **Important** Like the above changes in Axlib, the `AccountInfo` structure has also changed. It is recommended that chain developers explicitly add `AccountInfo: 'AccountInfoWithRefCount'` for the version with `refCount` on and `AccountInfoWithProviders` for the latest Axlib version. As per the above, the API defaults will be changed to align with Axlib.
...
- Updates to allychain types, ensuring it has coverage for all the latest
- Update all BetaNet types to the latest (session keys, session reports, allychain indexes)
- Update known upgrade checkpoints fo AlphaNet
- Add types of the lottery module in Axlib
...
Upgrade priority: Low. Fixes for allychain types, `.entries()` (with no values) and `event.is(...)` checks, users of these interfaces will have benefit.
...
- Add missing allychain types, `ParaGenesisArgs`
- Correct `EthTransaction` type (as per the correct cargo crate)
- Update with latest contract types for Axlib master
- Update alias mapping for asset palette (including Axlib starting defaults)
...
- Allow for use of `system.ss58Prefix` as exposed (current Axlib master)
- Correct use of api-contract `blockWeights.maxBlock` in estimation
- Build metadata docs & interfaces with section aliases
- Update to latest Axlib master metadata in tests


## 3.2.1 Dec 27, 2020

Upgrade priority: Low. Recommended for users of allychains.
...
- Additional allychain types (Thanks to https://github.com/joelamouche)
...
- Update allychain types to support BetaNet v1
- Adjust bounties derive to cater for new Axlib namespaces (& move to bounties)
- Mark `detectPackage` as known side-effect
- Add `detectPackage` to `@axia-js/metadata` (a number of requests on this)
- Adjust x-rxjs wrapper to expose all methods, incl. Node.js esm fixes
- Update to latest Axlib static metadata
- Update `@axia-js/util` to 5.1.1


## 3.0.1 Dec 14, 2020

Upgrade priority: Low. Recommended for the next AXIALunar/AXIA upgrades due to additional types and those following chains following Axlib master closely.
...
- Add new Axlib consumed Weight types
...
- Update `@axia-js/util` to 5.0.1


## 2.10.1 Dec 7, 2020

Upgrade priority: Low. Recommended when following Axlib master and/or allychains development, contains the latest types for both.
...
- Apply current AXIA master allychain types
- Apply current Axlib master asset types
- Add `system_{addLogFilter, resetLogFilter}` RPCs
- Adjust tests for latest Axlib metadata
...
- Add types for Axlib master node (to allow for consistent swapping to `MultiAddress`)
- Adjust `Bytes` with `.slice` to be consistent for `Uint8array` extensions (already released in 2.8.2 as a bugfix)


## 2.8.1 Nov 23, 2020

Upgrade priority: Low. Recommended if using Axlib master with new `MultiAddress`.
...
- Support new contract `Schedule` type as per Axlib
...
Upgrade priority: Low. Should be done alongside an update to `@axia-js/util` to 4.0
...
- Bump static v12 Metadata (tests) to latest Axlib version
- Bump `@axia-js/{util,util-crypto}` to 4.0 (optimized WASM layers)
...
- **Breaking change** Axlib changed the `ContractExecResult` from `api.rpc.contracts.call`. The API does have pass-through support for both old and new variants via the `Contract` interface, however it means the structure has changed to accommodate the new Axlib structure. Refer to the [docs for the latest interface](https://axia.js.org/docs/api-contract/start/contract.read) (including `isSuccess/isError` -> `isOk/isErr`, and the availability of the `gasConsumed`)
...
- Expand BetaNet known types to cater for v1 (specVersion 10)
...
- Add initial implementation for new Axlib `MultiAddress` type
...
- **Breaking change** `@axia-js/api-contract` now only supports ink! 3.0-rc1 and later formats. Users of ink! 2.1 are recommended to stay on an older version until upgraded.
...
- Adjust `ContractInfo` type for latest Axlib (contracts)
- Adjust `Schedule` type for latest Axlib (contracts)
...
- Remove all documentation after move to https://github.com/axia-js/docs repo
...
- Rename static TRANSACTION_VERSION to EXTRINSIC_VERSION (Axlib alignment)
- Bump Axlib metadata (tests, doc generation)


## 2.0.1 Sep 28, 2020

Upgrade priority: Medium. Required for all teams building on Axlib 2.0 for full compatibility.
...
- **Breaking change** Metadata versions before v9 as not supported anymore. v4 was the first version to go with the Axlib 1.0 while AXIALunar itself starts at v9 as the earliest version. This means that Axlib 1.x chains that have not had any upgrades are not supported in this version of the API anymore.
...
- Drop support for Axlib 1.0 metadata versions 1-8
- Drop support for Axlib 1.0 extrinsics version 1-3
- Add support for `grandpa_proveFinality` RPC
- Drop support for linked map queries not via .entries (deprecated since early versions of Axlib 2)
- Support `.entries()` on older chains without `queryStorageAt` support
- Add bounty type definitions from treasury palette
- Adjust `RefCount` type as per axlib 2.0
...
- Adjust staking derives to cater for early Axlib 2.0 chains (optional/non-optional EraIndexes)
...
- Static metadata & tests updated for the latest axlib master
- `toHuman()` & `.toBigInt()` has been explicitly added to the API documentation
- Adjust known types for latest AXIALunar network state
...
- **Important** Axlib changed the `RewardDestination` enum with an extra field for payout-to-any account for staking preferences. If on an older chain consider adding `RewardDestination: 'RewardDestinationTo257'`
- **Important** Axlib changed the `CompactAssignment` type enhanced types for session solution submissions. If on an older chain consider adding `CompactAssignments: 'CompactAssignmentsTo257'`
...
Upgrade priority: Low, unless using the `proposeAllychain` module.

Changes:

- Adjust balance retrievals to check for `system.account` for new/old determination
- Add types for the new `proposeAllychain` module (as per BetaNet)
...
- **Important** The Axlib `CompactAssignments` type has changed for better space utilization. On older chains, supply `CompactAssignments: 'CompactAssignmentsTo257'`
...
Upgrade priority: Low, unless using Centrifuge or allychains
...
- Update types for allychains
- `@axia-js/util` 3.1
- `@axia-js/wasm-crypto` 1.3


## 1.27.1 Aug 3, 2020

Upgrade priority: Low, unless using the BetaNet chain
...
- Add know types for the BetaNet chain
...
- Update metadata static tests for latest Axlib
- `@axia-js/util` 3.0
...
- **Important** Contract RPC result updated on Axlib, on older chains supply `ContractExecResult: 'ContractExecResultTo255'`
...
- **Important** Axlib changed the treasury tip structure, on older chains supply `OpenTip: 'OpenTipTo225'`
...
- Add new Axlib types for Babe `EquivocationProof`
- Adjust `Releases` enum to use a shared instance


## 1.20.1 Jun 22, 2020

- **Important** RPC for `account_nextIndex` has been moved to `system_accountNextIndex` (Aligning with Axlib as a primary alias)

Changes:

- Cleanup map entries support with only Axlib 2.0-era RPCs (when introduced)
...
- Adjust types & metadata for latest Axlib
- `@axia-js/util` 2.15
...
- Align metadata and types with latest Axlib
- Update AXIA types for current evolution of allychains
...
- Add types for new Axlib proxy module
- Adjust types for new Axlib (renamed) npos-elections module
- `Call` now exposes `method` & `section` fields (aligned with `toHuman()` representation)
- Cater for new Axlib string subscription ids (with backwards compatibility)
- `@axia-js/util` 2.13
...
- Support for latest Axlib types & metadata
- `@axia-js/util` 2.12
...
- Adjust `Multiplier` type to `Fixed128` (as per latest Axlib)
- Updated to latest Axlib metadata & add new types
...
- Align imOnline `Heartbeat` with Axlib (Thanks to https://github.com/arjanz)
...
- Remove static test-only metadata for AXIA dev chains (not updated regularly, Axlib dev to remain)

Changes:

- Revert error swallow for wrong preimage data (temp. override for AXIALunar upgrade with no migration)
- Fix `.encodedLength` calculation on `[Type; N]` types
- Add new `rpc.grandpa.roundState` RPC
- Update types & metadata as per latest Axlib master
...
- Update `@axia-js/util` to stable 2.9.1 (sadly missed in the previous version, stable should match to stable)
...
- Allow subscription to all storage keys, alignment with Axlib (Thanks to https://github.com/xlc)
...
- **Important** Axlib master has an updated `Weight` (to `u64`), on older chains apply `Weight: 'u32'` (This does affect event decoding if wrongly set)
...
- Adjust types for AXIALunar 1057 (notably the new `StakingLedger`)


## 1.10.1 Apr 13, 2020

- **Important** Axlib master has updated staking, on older chains apply `StakingLedger: 'StakingLedgerTo240'`

Changes:

- Add `rpc.system.chainType` to retrieve the chain type (Development. Local, Live & Custom)
- Adjust types for AlphaNet after the reset
...
- Add Centrifuge base types to `@axia-js/types-known`
- Adjust with all the latest axlib master types
...
- Update metadata/docs-from-metadata to the latest Axlib
...
- **Breaking change** (derives only) Additional cleanups of democracy proposal & referenda derives, including support for new AXIA/Axlib vote retrievals

Changes:

- Tx signing via `signAndSend` & `signAsync` now tries to use the last finalized block (depending on finalized drift)
- Add support for `api.rpc.childstate.*` Axlib RPCs, older nodes with still populate `api.rpc.state.*` with the child methods
- Adjust `Error.message` on RPC failures to not include the (already known) method signature
- Split `@axia-js/types/known` into `@axia-js/types-known`, allowing for base API overrides
...
- Internal adjustment of interface type locations (no impact on imports via `@axia-js/types/interfaces`)


## 1.8.1 Mar 22, 2020

- **Breaking change** The format for any custom RPCs have been changed (alongside API-internal changes) to allow for better RPC management. If you are currently using custom RPCs (or planning to do so), look at the [updated documentation](https://axia.js.org/api/start/rpc.custom.html)
- **Breaking change** Alongside API RPC changes, the `@axia-js/jsonrpc` package has been removed. Since it was never documented and only used internally, this should not have adverse impacts. All RPC definitions itself has now been moved to the relevant modules inside `@axia-js/types/interfaces`
- **Important** Axlib has an updated democracy module. If using an older chain add the `ReferendumInfo: 'ReferendumInfoTo239'` type when using referendums

Changes:

- The `isRetracted` Extrinsic status is now a warning, not a fatal error, correctly aligning with AXIA/Axlib
- The Axlib extrinsic phase definitions has been expanded with `Initialization` to align with the latest versions
- Add support for all known axlib RPC methods
...
- **Important** Current versions of AXIA/Axlib have dropped support for linked maps on storage entries. If you are using these queries to retrieve all entries, for instance `staking.{nominators,validators}()` and using an upgraded chain, you need to swap to retrieving entries via `.entries()` or `.keys()` for the keys.

Changes:

- Pull in support and types for latest AXIA/Axlib
- Add support for the new Axlib `state_getKeysPaged` RPC, including use in storage keys
...
- Added `derive.eras*` interfaces for queries to new Axlib staking interfaces
...
- **Important** Axlib master has updated staking, on older chains supply `StakingLedger: 'StakingLedgerTo223'`
- **Important** Axlib master has updated collective, on older chains supply `Votes: 'VotesTo230'`
...
- Add types & metadata for the latest AXIA/Axlib runtime versions (master branches)
...
- Update `@axia-js/{util, util-crypto, keyring}` to 1.6.1 & `@axia-js/wasm` to 1.2.1
...
- Move `DigestItem`, `Fixed64`, `H160`, `H256` & `H512` to interfaces in `@axia-js/types/interfaces`
...
- Remove [11 Jan 2019](https://github.com/axia-js/api/issues/574) extrinsic length hack
...
- Bump to `@axia-js/{util, util-crypto, keyring}` 2.5.1
...
- Type generation scripts (including `axia-types-from-{chain,defs}`) has been moved to the `@axia-js/typegen` package
- The `@axia-js/types/codec/create` has been moved to `@axia-js/types/create`, imports from `@axia-js/types` are unaffected
...
- **Important** Update RPC status codes (latest Axlib 2), with the `isInBlock` (`isFinalized` now indicates finality)
...
- Add support for `system.account` for balance & nonce queries in derives as well as Submittables (Axlib composites)
- Add `rpc.author.hasKey` and `rpc.author.hasSessionKeys` RPCs
- Add derives for the allychains interfaces
- Applied the latest types for AXIA allychains
...
- Add typings for `api.tx.*.*` - the `axia-types-from-chain` now also generates these
...
- Adjust balances fees derivation with optional transfer and creation fees (removed in Axlib)
- Adjust enum `.toJSON()` to return type name as specified


## 1.0.1 Jan 30, 2020

- **Breaking change** Drop support for Axlib v1 chain in all derives
...
- Add `axia-types-from-chain` & `axia-types-from-defs` TypeScript generators to `@axia-js/types`
...
- Update metadata and types for the latest Axlib & AXIA versions
- Support metadata V11 as per latest Axlib
- Remove support for un-deployed metadata
- Remove old known types for Axlib V1


## 0.100.1 Jan 13, 2020

- **Important** This will the the last API version with Axlib 1.x support. Although you will still be able to use subsequent versions with older chains, dependent libraries such as sr25519 may not be compatible.
...
- Add support for the Axlib identity module
- Remove the `codec/Data` type, to remove a conflict with Axlib. This type is now named `Raw`
- Fix for linked maps using `Option`
- Add support for Metadata v10
- Add support for latest AXIA/Axlib types
...
- Alignment with latest AXIA/Axlib master branch types
...
- Support for new Axlib democracy with preimages in derive
- Alignment of types with Axlib/AXIA master branches
...
- **Breaking change** The metadata classes have been moved from `@axia-js/types` into `@axia-js/metadata`. If you explicitly create `Metadata` classes, you now would need to do so with the `import Metadata from '@axia-js/metadata';` This is a decorated class, where the previous metadata is now available on `Metadata.metadata`, alongside decorated `consts.*`', `tx.*` and `query.*`.
- **Breaking change** Session `Keys` defaults to 4 keys now (Axlib master), depending on your node config, you would want to override with e.g. `Keys: 'SessionKeys3'` (1, 2, 3, 4, 5 & 6)
...
- Aligned all types with the latest AXIA & Axlib, including AXIALunar CC3
...
- Updated types for latest AXIALunar
...
- Update council derives for the latest Axlib master support (with constants)
...
- Support Extrinsic V4 with additional signature indicator (Latest AXIA/Axlib/AXIALunar)
...
- All latest AXIA & Axlib types (as per master branches)
...
- Update to latest axlib master metadata
...
- For TypeScript users `api.query.*.*` is now properly typed for default Axlib calls, i.e. `api.query.balances.freeBalance(...)` will return a `Balance` type. Additionally the `api.queryMulti` and `.multi` on queries not allow generic type params.
...
- Updated types to support the latest AXIA/Axlib master changes
...
- Support the latest AXIA & Axlib master branches (incl. metadata updates)
...
- Adjustments for Axlib 1.x chain detection (with auto-types) and Axlib 2.x support has been extended with all latest types
- The `getRuntimeVersion` and `subscribeRuntimeVersion` RPCs are now only available on the `rpc.state.*` endpoints. This aligns with the Axlib implementation.
- The `author_insertKey` RPC's last argument `publicKey` is now required, as to reflect Axlib implementation.
- Support for extrinsics with versions that is not in the base Axlib implementation (V1-V3) can now be done by providing an implementation for `ExtrinsicUnknown`
...
- `Extrinsic`, `ExtrinsicPayload` & `SignerPayload` is registered in the type registry and can be overridden now
  ...
  - **Breaking change** `SignerPayloadJSON`, `SignerPayloadRawBase` and `SignerPayloadRaw` are all moved to `@axia-js/types`
...
- **Breaking change** Runtime types have been extended and moved to definitions instead of classes
  ...
  - `Moment` now implements as `u64` as per the Axlib codebase. (It it up to the user to interpret, for axlib 2.x it is not ms resolution)
  ...
  - For moved types, a previous import would have been from `@axia-js/types`, i.e. `import { SetIndex } from '@axia-js/types`, now just the interfaces (TypeScript) are available via `import { SetIndex } from '@axia-js/types/interfaces`
...
- The `types/codec/createType` has been moved to `types/codec/create` with the same exports. If you explicitly imported from here (and not doing the suggested `import { createType } from '@axia-js/types`), the reference needs to be updated
...
- Support for axlib 2.x (master) has been extended,
  - Additional types have been addedd for the modules
  - `api.derive.contract` is now `api.derive.contracts` to align with the axlib 2.x rename. (Feature detection is used so it supports both 1.x and 2.x chains)
  - Addition of `api.derive.elections`

- Support latest axlib 2 v6 metadata with module constants using `api.consts`.
  ...
  - Internally the `@axia-js/extrinsics` and `@axia-js/storage` have been moved to `@axia-js/api-metadata` and are now accessible as `@axia-js/api-metadata/extrinsics` and `@axia-js/api-metadata/storage`, respectively.
...
- The `ContractsAbi` type has been moved from `@axia-js/types` to `import { Abi } from '@axia-js/api-contract`. This paves the way for an enhanced contracts interface, instead of dealing with low-level API calls.
...
- The dependency on `@axia-js/keyring` has been removed - if you relied on the API to provide this, you would now need to manually add it
...
- Support latest axlib 2.x v5 metadata
  - Add `ApprovalFlag`, `SetIndex`, `VoterInfo` types for council as per axlib
  - Update `ContractInfo` to match axlib master

- Add support for new inherent digests from axlib.
  - **Breaking change** Because of the new `Seal`, the old/new Seals are not compatible. If you were using the old `Seal` from the `Digest`s, you would now use `isSealV0` for the pre-axlib 1.0 version and the new `isSeal` for the current version
  - Update `HeaderExtended` to retrieve author information from axlib 2.x (new `PreRuntime` digests)
...
- Updated metadata for current axlib master
...
- Support latest EventRecord metadata (axlib master)
...
- Metadata updates for axlib (WithdrawReasons, ContractInfo)
...
- Update @axia-js/keyring to enable Alice's stash account on dev chains
- Update @axia-js/util-crypto with smaller footprint
...
- Change spelling to US English as per axlib master (1.0-rc1). Breaking changes as a result:
...
- Align types as per latest axlib master
- PeerInfo from system_peers does not have the index field anymore (dropped in axlib)
...
- Support metadata V2 as per latest axlib master
- Update metadata with new types as per latest axlib master
...
- Update upstream @axia dependencies (for new crypto)
...
- Where `Extrinsic` and `Method` is used as types, consider importing `{ IMethod, IExtrinsic }` from `@axia-js/types/types`, especially in the cases where this is used from a `SubmittableExtrinsic`
...
```js
// old (affected)
import Method from '@axia-js/types/Method';
import Signature from '@axia-js/types/Signature';

// new locations
import Method from '@axia-js/types/primitive/Method';
import Signature from '@axia-js/types/type/Signature';

// unaffected
import { Method, Signature } from '@axia-js/types';
```
...
- Introduces support for the new keyring with sr25519 support in addition to ed25519. While this does not change the exposed API, it is considered breaking since @axia-js/keyring has interface changes. (Unless needed, don't rush the upgrade)
...
- Support for axlib hash signing with implVersion >= 18
...
- Support the V1 metadata specification from Axlib in addition to the currently testnet active V0 version
...
Axlib has been updated with a breaking new transaction format where the Index/Nonce is now encoded as a Compact. This change is being rolled out to both Alexander (AXIA testnet) as well as Charred Cherry (Axlib testnet) - transactions between old and new are not compatible.
...
Swapped to new metadata structures from Axlib. If the API is not working with your node, update Axlib to latest master branch. (Or 0.9.1 for Charred Cherry). Dropped support for old metadata as found as far back as BBQ Birch.
