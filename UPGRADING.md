...
```js
import { Struct, Text, u32 } from '@axia-js/types';
...
In the current iteration, the underlying `@axia-js/types` bases structures now require a `Registry` to be passed as the first parameter. This means that the above signature would be adjusted to -
...
Previously, when creating a type instance such as `BlockNumber`, you would do `api.createType('BlockNumber', <initValue>)`, this is unchanged. In the cases where you directly import from `@axia-js/types`, the following pattern is required -

```js
import { createType } from '@axia-js/types';
...
import { TypeRegistry, createType } from '@axia-js/types';
...
The 0.90.1 release caters for the [AXIALunar network](https://axialunar.network/) and pulls in all the changes to support [Axlib 2.x](https://github.com/axia-tech/axlib), all while maintaining backwards compatibility to allow operation on networks such as [AXIA's Alexander](https://axia.network/).
...
There will no doubt be breakages in using calls to now non-existent endpoints (as populated by the metadata) if you are upgrading your nodes to Axlib 2.x. Axlib 2.x has had a number of internal changes, where new modules and features are introduced (such as `babe` and `technicalCommittee`), some modules have been renamed (such as `contract` -> `contracts`) and modules such as `session` has been reworked to a large degree.

To cater for both 1.x and 2.x support, the [@axia-js/api-derive](packages/api-derive) endpoints, do feature detection for the node type and should continue working as-is. Additionally, a number of new derives have been added, specifically around elections.

### Type renames

To better align with the actual types from the metadata, and avoid (too much) context switching, some types from the `@axia-js/types` have been renamed. These include -
...
The [@axia-js/api](packages/api) has always handled the conversion of types for parameters when making calls or queries. For example, when making a transfer to `BOB` (address), any of the following is valid -
...
First the rationale behind this - in all cases Axlib is very flexible, so while AXIA (and the Axlib base), define `type Balance = u128`, this can be different between chains. (This also applies to the majority of built-in supported types). As such, type construction should be done via the actual registered types.
...
The impact of this will be noticeable, if you have been importing the old-style type classes from `@axia-js/types`, those imports are not available anymore. For creation, just pass everything through the `createType`.

If a TypeScript user, you can find the updated type (it is a type definition only, not a class), under `@axia-js/types/interfaces`. To do type casting, using interfaces -

```js
// import the TypeScript runtime interfaces we wish to use
import { Balance, Hash } from '@axia-js/types/interfaces';

// import the primitives we wish to use
import { createType, Compact, Vec, u32 } from '@axia-js/types';
...
One of the major pain points in working with a custom Axlib node is the definition of types to cater for chains. There are 2 approaches: defining types via a JSON format or extending your own classes in TypeScript (or JS) and injecting these. For the latter category, there are some impacts in the way you define these.
...
For the definition of any structures using the Axlib specific types as classes, some adjustments are needed. Since the base modules types are now not available in classes, however it is needed for definitions, the following approach is encouraged -
...
import { ClassOf, Struct, u32 } from '@axia-js/types';
...
Internally the [@axia-js/types](packages/types) package now only defines classes where there are specific encoding logic applied. For all other types, the definitions are done via a JSON-like format and then the TypeScript definitions are generated from these. (In a world where nodes inject types and the type definitions are not needed, this functionality will be useful to allow TS developers to auto-generate type definitions based on what the node defines.)

### Signing transactions (Signer interface)

For users of the API signer interfaces (such as extensions and mobile signers), the interfaces have undergone some changes to cater for the extrinsic v2 format as defined by Axlib. If you are only supporting current chains (e.g. Alexander), no changes are required, however the old `sign` interface does not support chains such as AXIALunar, so all users are encouraged to upgrade to the new `signPayload` interface.

This has already been implemented in both the [axia-js extension](https://github.com/axia-js/extension/blob/5f22f67d558655c605eb6f6beecef6826ed6c159/packages/extension/src/page/Signer.ts#L16v) as well as the [simple single signer](https://github.com/axia-js/api/blob/d56905d1b566be6f17eb570ac01448378fc91b67/packages/api/test/util/SingleAccountSigner.ts#L37).
